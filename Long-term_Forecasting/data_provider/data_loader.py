...
class Dataset_ETT_hour(Dataset):
    def __init__(self, root_path, flag='train', size=None,
        ...
        self.set_type = type_map[flag]  # 训练集验证集测试集分别采用不同的编号
        ...
        self.tot_len = len(self.data_x) - self.seq_len - self.pred_len + 1  # 单个变量可用的时间序列的长度
    ...
    def __read_data__(self):
        ...
        border1s = [0, 12 * 30 * 24 - self.seq_len, 12 * 30 * 24 + 4 * 30 * 24 - self.seq_len]  # 定义三组边界值，分别用于训练、验证和测试
        border2s = [12 * 30 * 24, 12 * 30 * 24 + 4 * 30 * 24,
                    12 * 30 * 24 + 8 * 30 * 24]  # 根据数据集类型(train/val/test)选择对应的边界
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]

        if self.set_type == 0:
            border2 = (border2 - self.seq_len) * self.percent // 100 + self.seq_len  # 允许使用部分训练数据（由percent参数控制）

        if self.features == 'M' or self.features == 'MS':  # 多变量情形下
            cols_data = df_raw.columns[1:]  # 除第一列外的所有列
            df_data = df_raw[cols_data]
        elif self.features == 'S':  # 单变量
            df_data = df_raw[[self.target]]  # 只选择目标变量

        if self.scale:
            train_data = df_data[border1s[0]:border2s[0]]  # 使用训练集数据进行拟合
            self.scaler.fit(train_data.values)
            data = self.scaler.transform(df_data.values)  # 转换所有数据
        else:
            data = df_data.values

        df_stamp = df_raw[['date']][border1:border2]  # 取出时间戳
        df_stamp['date'] = pd.to_datetime(df_stamp.date)  # 把date列中的日期处理成统一的格式
        if self.timeenc == 0:
            ...
            data_stamp = time_features(pd.to_datetime(df_stamp['date'].values), freq=self.freq)  # 这里TimeFeature 类的
            # __call__ 方法期望输入是 pd.DatetimeIndex 类型，如果直接使用df_stamp则得到的是dataframe的格式，没有时间属性访问
            data_stamp = data_stamp.transpose(1, 0)  # 时间特征的处理，包含四列，分别表示HourOfDay, DayOfWeek, DayOfMonth, DayOfYear
    ...
    def __getitem__(self, index):
        feat_id = index // self.tot_len  # 确定是在针对哪一个feature进行操作
        s_begin = index % self.tot_len  # 这个batch的数据从那个数据开始

        s_end = s_begin + self.seq_len  # 输入序列结束位置
        r_begin = s_end - self.label_len  # 输出序列开始位置
        r_end = r_begin + self.label_len + self.pred_len  # 输出序列结束位置（会包含label_len与pred_len两个部分）
        seq_x = self.data_x[s_begin:s_end, feat_id:feat_id + 1]  # 取对应的输入输出数据集
